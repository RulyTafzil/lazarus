...
"""Email/account/header helpers shared by NED and clients.

Originally part of the 609-line ``lazarus.util`` grab-bag; the HTML/text
rendering and mail-content helpers have their own focused modules
(:mod:`ned.html_utils`, :mod:`ned.mail_utils`), re-exported here for
compat, and the Qt-dependent ``keys`` helpers stay in the desktop package
(``lazarus.keys``, re-exported by ``lazarus.util``).

New code should import from the owning module directly:

* HTML/text:  :mod:`ned.html_utils` (linkify, colorize, html2text, …)
* Mail parts: :mod:`ned.mail_utils` (message_parts, body_text, write_attachments, …)
* This file:  email identity + message helpers (strip/parse addresses,
              header splitting, wrapping)
...
# stay here so ``from ned.util import X`` keeps working (and ``lazarus.util``
# re-exports them onward for the desktop's public surface).
...
def wrap_message(s: str) -> str:
    """Hard wrap message body using :func:`~ned.settings.wrap_column`
